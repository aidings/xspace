...
def _get_class_representations(
    ...
    ignore_labels : Sequence[str | int], optional
        List of labels to ignore.
        Defaults to None.
    labels_order : Sequence[str | int], optional
...
def _seq2arr(seq: Sequence[str | int | object]):
    """Convert sequence to numpy.ndarray.

    Parameters
    ----------
    seq : Sequence[str | int | object]
    ...
    seqs = []
    for value in seq:
        seqs.append(to_pil(value)) 
 
    if issubclass(type(seqs[0]), Image.Image):
        return np.asarray(seqs, dtype=type(seqs[0]))
    else:
