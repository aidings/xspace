...
def color_print(*datas):
    color = random.choice(list(TextColor))
    data = generate_color(*datas, color=color.value)
    print(data)
...
def print_table(table, title, theme=Table_Theme.RED, border_style=BorderStyle.SINGLE_LINE, rich_mode=False, hilight=[],
                alignment=Alignment.LEFT):
    if hilight != []:
        assert max(hilight) < len(title), "hilight index must < the len of row"
    cloum_max = []
    imax_len = max([len(t) for t in title])
    border_style = border_style.value
    for cloum in range(0, len(table[0])):
        max_len = max([imax_len]+[len(str(data[cloum])) for data in table])
