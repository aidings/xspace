...
class Markdown:
    ...
    def text(self, text, esc=False):
        self.fp.write(esc_format(text, esc=esc))
    
    def header(self, text, level:int, style='atx', esc=True):
        self.fp.write('\n' + header(text, level, style, esc=esc) + '\n')
    
    def italics(self, text, esc=True):
        self.fp.write(italics(text, esc=esc))
    
    def bold(self, text, esc=True):
        self.fp.write(bold(text, esc=esc))
    
    def inline_code(self, text):
        self.fp.write(inline_code(text))
    
    def code_block(self, text, lang=''): 
        self.fp.write(code_block(text, lang) + '\n')
    
    def link(self, text, url, title=''):
        self.fp.write(link(text, url, title))
    
    def image(self, text, url, title='', esc=True):
        self.fp.write(image(text, url, title, esc=esc))
    
    def unordered_list(self, items, esc=True):
        self.fp.write(unordered_list(items, esc=esc))
    
    def ordered_list(self, items, esc=True):
        self.fp.write(ordered_list(items, esc=esc))
    
    def blockquote(self, text, esc=True):
        self.fp.write(blockquote(text, esc=esc)) 

    def horizontal_rule(self, length=79, style="_"):
        self.fp.write(horizontal_rule(length, style))
    
    def strikethrough(self, text, esc=True):
        self.fp.write(strikethrough(text=text, esc=esc))
    
    def task_list(self, items, esc=True):
        self.fp.write(task_list(items, esc=esc) + '\n')
    
    def table_row(self, text_list, pad=None, esc=True):
        self.fp.write(table_row(text_list, pad, esc=esc) + '\n')
    
    def table_delimiter_row(self, number_of_columns, column_lengths=None):
        self.fp.write(table_delimiter_row(number_of_columns, column_lengths) + '\n')
