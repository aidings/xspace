...
def short_uuid(len=16):
    return str(shortuuid.uuid4())[:len]
...
def _create_tabs(
    ...
    force_b64 : bool, optional
        ...
        Defaults to False.
    tabs_order : Sequence[str | int], optional
...
def _create_img(
    ...
    ----------
    image : str | object
        Image object | string URL to local/external image file.
    label : str | int
    ...
    if type(image) is str | type(image) is str_:
    ...
    # if image is not a string it means its either PIL.Image | np.ndarray
    ...
    <div class="ipyplot-placeholder-div-%(0)s">
        <div id="ipyplot-content-div-%(0)s-%(1)s" class="ipyplot-content-div-%(0)s">
            <h4 style="font-size: 12px; word-wrap: break-word; color: %(4)s;">%(2)s</h4>
            %(3)s
            <a href="#!">
    ...
    """ % {'0': grid_style_uuid, '1': img_uuid, '2': label, '3': img_html, '4': text_color}  # NOQA E501
    return html
